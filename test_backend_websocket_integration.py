...
class BackendWebSocketIntegrationTest:
        ...
        for symbol in self.test_symbols:
            try:
                ...
                if len(ohlcv_data) >= 40:  # Should get at least 40 candles
                    print(f"✅ {symbol}: OHLCV data fetched ({len(ohlcv_data)} candles)")
                    print(f"   Latest price: ${ohlcv_data['close'].iloc[-1]:.2f}")
                    success_count += 1
                else:
                    print(f"❌ {symbol}: Insufficient OHLCV data ({len(ohlcv_data)} candles)")
    ...
    async def test_websocket_service(self):
        ...
        try:
            ...
            await self.websocket_client.subscribe_orderbook('tBTCUSD', orderbook_callback)
    ...
    async def test_strategy_integration(self):
        ...
        try:
            ...
            print(f"✅ EMA Strategy: {ema_signal.action} (confidence: {ema_signal.confidence:.2f})")
            ...
            print(f"✅ RSI Strategy: {rsi_signal.action} (confidence: {rsi_signal.confidence:.2f})")
    ...
    async def test_data_quality_validation(self):
        ...
        try:
            ...
            # Run validation
            is_valid, validation_message = self.live_data_service.validate_market_conditions(
                market_context
            )
    ...
    async def run_integration_test(self):
        ...
        test_results.append(("Strategy Integration", await self.test_strategy_integration()))
        test_results.append(("Data Quality Validation", await self.test_data_quality_validation()))
