...
# Import strategies
from backend.strategies import ema_crossover_strategy, rsi_strategy, fvg_strategy, sample_strategy
...
def analyze_strategies():
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            return jsonify({"error": f"Data must contain columns: {required_columns}"}), 400
        ...
        strategies_info = {
            "ema_crossover": {
                ...
                "parameters": {
                    ...
                    "buy_threshold": {"type": "float", "default": 0.0, "description": "Threshold for buy signals"},
                    "sell_threshold": {"type": "float", "default": 0.0, "description": "Threshold for sell signals"},
...
def backtest_strategy():
    ...
    try:
        ...
        for i in range(window_size, len(df)):
            ...
            try:
                ...
                if signal.action == "buy" and signal.confidence > 0.6:
                    position_size = balance * signal.position_size * 0.1  # Max 10% per trade
                    positions.append({
                        "type": "buy",
        ...
        final_balance = balance + sum(pos["size"] * df.iloc[-1]["close"] for pos in positions)
