...
def register(app):
    ...
    def start_bot_route():
        ...
        current_app.logger.info(f"🤖 [Backend] Current bot status before start: {bot_status}")
        ...
        try:
            ...
            current_app.logger.error(f"❌ [Backend] Bot status after error: {bot_status}")
            import traceback
            current_app.logger.error(f"❌ [Backend] Stack trace: {traceback.format_exc()}")
            
            return jsonify({"error": f"Failed to start bot: {str(e)}", "success": False}), 500

    @app.route("/api/stop-bot", methods=["POST"])
    def stop_bot_route():
        """Stoppar tradingboten."""
        current_app.logger.info("🤖 [Backend] STOP bot request received")
        current_app.logger.info(f"🤖 [Backend] Current bot status before stop: {bot_status}")
        ...
        try:
            ...
            return jsonify({"error": f"Failed to stop bot: {str(e)}", "success": False}), 500
