...
class LivePortfolioServiceAsync:
    ...
    async def get_live_portfolio_snapshot(
        ...
        try:
            ...
            for symbol in symbols_to_use:
                ...
                amount = 0.5 if symbol == "tBTCUSD" else (2.0 if symbol == "tETHUSD" else 10.0)
                entry_price = 35000.0 if symbol == "tBTCUSD" else (2000.0 if symbol == "tETHUSD" else 150.0)
                current_price = 36000.0 if symbol == "tBTCUSD" else (2100.0 if symbol == "tETHUSD" else 155.0)
            ...
            total_unrealized_pnl_pct = (total_unrealized_pnl / (total_value - total_unrealized_pnl)) * 100 if total_value > total_unrealized_pnl else 0
    ...
    async def get_portfolio_performance(self, timeframe: str = "24h") -> List[PerformanceMetric]:
        """
        ...
        try:
            ...
            if timeframe == "1h":
                return [
                    PerformanceMetric(name="Return", value=0.5, unit="%", timestamp=now),
                    PerformanceMetric(name="Volatility", value=2.3, unit="%", timestamp=now),
                    PerformanceMetric(name="Sharpe Ratio", value=1.2, unit="", timestamp=now),
                    PerformanceMetric(name="Max Drawdown", value=0.8, unit="%", timestamp=now),
                ]
            elif timeframe == "24h":
                return [
                    PerformanceMetric(name="Return", value=2.1, unit="%", timestamp=now),
                    PerformanceMetric(name="Volatility", value=3.5, unit="%", timestamp=now),
                    PerformanceMetric(name="Sharpe Ratio", value=1.5, unit="", timestamp=now),
                    PerformanceMetric(name="Max Drawdown", value=1.2, unit="%", timestamp=now),
                ]
            elif timeframe == "7d":
                return [
                    PerformanceMetric(name="Return", value=5.8, unit="%", timestamp=now),
                    PerformanceMetric(name="Volatility", value=4.2, unit="%", timestamp=now),
                    PerformanceMetric(name="Sharpe Ratio", value=1.8, unit="", timestamp=now),
                    PerformanceMetric(name="Max Drawdown", value=2.5, unit="%", timestamp=now),
                ]
            else:  # 30d eller annat
                return [
                    PerformanceMetric(name="Return", value=12.3, unit="%", timestamp=now),
                    PerformanceMetric(name="Volatility", value=5.1, unit="%", timestamp=now),
                    PerformanceMetric(name="Sharpe Ratio", value=2.1, unit="", timestamp=now),
                    PerformanceMetric(name="Max Drawdown", value=4.2, unit="%", timestamp=now),
                ]
    ...
    async def validate_trade(
        ...
        try:
            ...
            price = 36000.0 if symbol == "tBTCUSD" else (2100.0 if symbol == "tETHUSD" else 155.0)
            ...
            if trade_type == "buy":
                ...
                message = (
                    "Trade is valid" if is_valid else 
                    f"Insufficient balance. Required: {trade_cost}, Available: {available_balance}"
                )
            else:  # sell
                # Simulera position för symbolen
                position_amount = 0.5 if symbol == "tBTCUSD" else (2.0 if symbol == "tETHUSD" else 10.0)
                max_trade_size = position_amount
                is_valid = amount <= position_amount
                message = (
                    "Trade is valid" if is_valid else 
                    f"Insufficient position. Required: {amount}, Available: {position_amount}"
                )
