...
class LiveDataService:
    ...
    def __init__(self, exchange_id: str = 'bitfinex', api_key: str = None, api_secret: str = None):
        """
    ...
    def fetch_live_ohlcv(self, symbol: str, timeframe: str = '5m', limit: int = 100) -> pd.DataFrame:
        """
        ...
        """
        try:
            logger.info(f"🔴 [LiveData] Fetching live OHLCV: {symbol} {timeframe} (limit: {limit})")
            ...
            logger.info(f"✅ [LiveData] Price range: ${df['low'].min():.2f} - ${df['high'].max():.2f}")
    ...
    def fetch_live_ticker(self, symbol: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"✅ [LiveData] Ticker fetched - Price: ${ticker['last']:.2f}, Volume: {ticker['baseVolume']:.4f}")
    ...
    def fetch_live_orderbook(self, symbol: str, limit: int = 20) -> Dict:
        ...
        """
        try:
            logger.info(f"📚 [LiveData] Fetching live orderbook: {symbol} (limit: {limit})")
            ...
            logger.info(f"✅ [LiveData] Orderbook fetched - Bid: ${best_bid:.2f}, Ask: ${best_ask:.2f}, Spread: ${spread:.2f}")
    ...
    def get_live_market_context(self, symbol: str, timeframe: str = '5m', limit: int = 100) -> Dict:
        """
        ...
        try:
            ...
            logger.info(f"✅ [LiveData] Price: ${latest_close:.2f}, Volume: {volume_24h:.4f}, Volatility: {price_std:.2f}%")
            
            return market_context
            
        except Exception as e:
            logger.error(f"❌ [LiveData] Failed to get market context for {symbol}: {e}")
            raise
    ...
    def validate_market_conditions(self, market_context: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            spread_pct = (market_context['spread'] / market_context['current_price']) * 100
            ...
            if market_context['volatility_pct'] > 50:  # 50% volatility threshold
                return False, f"High volatility: {market_context['volatility_pct']:.2f}%"
