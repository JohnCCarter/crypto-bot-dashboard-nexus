...
def main():
    ...
    if trading_window.is_within_window() and trading_window.can_trade():
        logger.info("🕐 [TradingBot] Trading window is OPEN - proceeding with live analysis")
        ...
        try:
            ...
            is_valid, validation_message = live_data.validate_market_conditions(market_context)
            
            if not is_valid:
                logger.warning(f"⚠️ [TradingBot] Market conditions not suitable: {validation_message}")
                notifier.send(f"Trading paused - Market conditions: {validation_message}")
                return
            
            logger.info(f"✅ [TradingBot] Market validation passed: {validation_message}")
            ...
            logger.info(f"   EMA strategy: {ema_signal.action} (confidence: {ema_signal.confidence:.2f})")
            logger.info(f"   RSI strategy: {rsi_signal.action} (confidence: {rsi_signal.confidence:.2f})")
            logger.info(f"   FVG strategy: {fvg_signal.action} (confidence: {fvg_signal.confidence:.2f})")
            ...
            logger.info(f"   EMA position: {ema_position_size:.6f} {symbol.split('/')[0]}")
            logger.info(f"   RSI position: {rsi_position_size:.6f} {symbol.split('/')[0]}")
            logger.info(f"   FVG position: {fvg_position_size:.6f} {symbol.split('/')[0]}")
            ...
            if executing_trades:
                logger.info(f"🚀 [TradingBot] EXECUTING {len(executing_trades)} trades based on live data:")
                
                for trade in executing_trades:
                    logger.info(f"   📈 {trade['strategy']}: {trade['action'].upper()} "
...
⏰ Timestamp: {market_context['timestamp']}
                ...
                notifier.send(f"Trading bot analyzed live data - no trades executed. "
        ...
        notifier.send("Trading bot checked - outside trading window or max trades reached.")
