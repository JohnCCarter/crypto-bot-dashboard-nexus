...
class PortfolioManagerAsync:
    ...
    async def process_signals(self, signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for signal in signals:
            ...
            signals_dict = {
                ...
                "metadata": {
                    "probability_buy": signal.get("indicators", {}).get("probability_buy", 0.33),
                    "probability_sell": signal.get("indicators", {}).get("probability_sell", 0.33),
                    "probability_hold": signal.get("indicators", {}).get("probability_hold", 0.34),
                }
            }
        ...
        for symbol, strategy_signals in symbols_signals.items():
            ...
            price = next(
                (s.get("price", 0.0) for s in signals if s.get("symbol") == symbol), 
                0.0
            )
    ...
    async def calculate_allocations(
        ...
        buy_actions = [a for a in actions if a["action"] == "buy" and a["should_execute"]]
        ...
        for action in buy_actions:
            if total_buy_confidence > 0:
                ...
                allocation_percent = allocation_weight * max_allocation_percent * risk_factor
    ...
    async def get_portfolio_summary(
        ...
        # Get risk assessment
        risk_assessment = await self.risk_manager.assess_portfolio_risk(current_positions)
    ...
    async def rebalance_portfolio(self, target_allocations: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for target in target_allocations:
            ...
            target_pct = target.get("percentage", 0) / 100 if isinstance(target.get("percentage"), float) else 0
...
async def get_portfolio_manager_async(
    ...
    logging.info("PortfolioManagerAsync initialized with %d strategies", len(strategy_weights))
