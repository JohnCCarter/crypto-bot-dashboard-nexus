...
class LivePortfolioService:
    ...
    def get_live_portfolio_snapshot(self, symbols: List[str] = None) -> LivePortfolioSnapshot:
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    if position_amount > 0:
                        ...
                        logger.info(f"📈 [LivePortfolio] {symbol}: {position_amount:.6f} @ ${current_price:.2f} "
                                  f"(PnL: ${unrealized_pnl:.2f})")
                    
                    market_data_quality[symbol] = True
                    
                except Exception as e:
                    logger.error(f"❌ [LivePortfolio] Failed to get live data for {symbol}: {e}")
            ...
            logger.info(f"   Total PnL: ${total_unrealized_pnl:.2f} ({total_unrealized_pnl_pct:.2f}%)")
            
            return snapshot
            
        except Exception as e:
            logger.error(f"❌ [LivePortfolio] Failed to generate portfolio snapshot: {e}")
            raise
    ...
    def get_position_value(self, symbol: str, amount: float) -> Dict:
        ...
        """
        try:
            logger.info(f"💰 [LivePortfolio] Calculating position value for {amount:.6f} {symbol}")
    ...
    def validate_trading_capacity(self, symbol: str, trade_amount: float, 
                                trade_type: str) -> Dict:
        """
        ...
        """
        try:
            logger.info(f"🔍 [LivePortfolio] Validating {trade_type} of {trade_amount:.6f} {symbol}")
    ...
    def get_portfolio_performance_metrics(self, timeframe: str = '24h') -> Dict:
        ...
        """
        try:
            logger.info(f"📊 [LivePortfolio] Calculating performance metrics for {timeframe}")
            ...
            metrics = {
                ...
                'cash_allocation_pct': (current_snapshot.available_balance / current_snapshot.total_value) * 100,
                'position_count': len(current_snapshot.positions),
                'largest_position': max([pos.market_value for pos in current_snapshot.positions]) if current_snapshot.positions else 0,
