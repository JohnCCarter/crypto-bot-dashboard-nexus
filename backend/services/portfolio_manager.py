...
class PortfolioManager:
    ...
    def __init__(self, risk_manager: RiskManager, strategy_weights: List[StrategyWeight]):
        """
    ...
    def _validate_weights(self):
        ...
        enabled_weights = [sw.weight for sw in self.strategy_weights.values() if sw.enabled]
        total_weight = sum(enabled_weights)
        
        if total_weight > 1.1 or total_weight < 0.9:
            logging.warning(f"Strategy weights sum to {total_weight:.2f}, consider normalizing")
    ...
    def calculate_portfolio_position_size(
        ...
        metadata.update({
            ...
            "portfolio_diversification_factor": self._calculate_diversification_factor(current_positions)
        })
        
        return position_size, metadata
    
    def _calculate_diversification_factor(self, current_positions: Dict[str, Dict[str, Any]]) -> float:
    ...
    def should_execute_trade(
        ...
        validation = self.risk_manager.validate_order_with_probabilities(
            order_data, portfolio_value, current_positions, combined_signal.combined_probabilities
        )
        ...
        if portfolio_risk["risk_level"] == "high" and combined_signal.combined_probabilities.get_risk_score() > 0.6:
            execution_decision = False
            validation["errors"].append("Portfolio risk too high for additional positions")
    ...
    def rebalance_portfolio_weights(self, performance_data: Dict[str, float]):
        ...
        for strategy_name, performance in performance_data.items():
            if strategy_name in self.strategy_weights:
                ...
                logging.info(f"Adjusted {strategy_name} weight: {current_weight:.2f} -> {new_weight:.2f}")
    ...
    def _normalize_weights(self):
        ...
        enabled_strategies = {name: sw for name, sw in self.strategy_weights.items() if sw.enabled}
    ...
    def get_portfolio_summary(self, current_positions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "enabled_strategies": sum(1 for sw in self.strategy_weights.values() if sw.enabled),
