...
class ConfigService:
    ...
    def validate_config(self) -> List[str]:
        ...
        try:
            ...
            if risk.get("max_position_size", 0) <= 0 or risk.get("max_position_size", 0) > 1:
            ...
            enabled_strategies = [
                name for name, cfg in config.portfolio_strategies.items() 
            ...
            total_weight = sum(
                cfg.get("weight", 0) for cfg in config.portfolio_strategies.values()
                if cfg.get("enabled", False)
            )
            ...
            for threshold in ["confidence_threshold_buy", "confidence_threshold_sell", "confidence_threshold_hold"]:
        ...
        try:
            ...
            return {
                ...
                "probability_framework": {
                    "combination_method": config.probability_settings.get("combination_method"),
                    "dynamic_weights_enabled": config.probability_settings.get("enable_dynamic_weights"),
                    "risk_threshold": config.probability_settings.get("risk_score_threshold")
                }
            }
