...
class EventLogger:
    ...
    def log_event(self, event_type: EventType, message: str, 
        ...
        log_message = (f"🎯 [{timestamp}] {event_type.value.upper()}: "
                       f"{message}")
    ...
    def log_trade(self, symbol: str, side: str, amount: float, price: float, 
                  order_id: str, strategy: Optional[str] = None):
    ...
    def log_order_creation(self, symbol: str, side: str, amount: float, 
                           price: float, order_type: str):
        """Logga order creation"""
        message = (f"Created {order_type} {side} order: "
    ...
    def log_bot_action(self, action: str, 
                       details: Optional[Dict[str, Any]] = None):
        """Logga bot start/stop"""
        event_type = (EventType.BOT_STARTED if action == "start" 
                      else EventType.BOT_STOPPED)
        self.log_event(event_type, f"Trading bot {action}ed", details)
        
    def log_parameter_change(self, parameter: str, old_value: Any, 
                             new_value: Any):
        """Logga parameter ändringar"""
        message = (f"Parameter '{parameter}' changed: "
                   f"{old_value} -> {new_value}")
        self.log_event(
            EventType.PARAMETER_CHANGED,
            message,
            {"parameter": parameter, "old": str(old_value), 
             "new": str(new_value)}
        )
    ...
    def log_backtest(self, strategy: str, symbol: str, 
                     result: Dict[str, Any]):
...
def log_user_action_only(endpoint: str, method: str, message: str, 
                         details: Optional[Dict[str, Any]] = None):
    """
    ...
    if not should_suppress_routine_log(endpoint, method):
        event_type = (EventType.TRADE_PLACED if method == "POST" 
                      else EventType.API_ERROR)
