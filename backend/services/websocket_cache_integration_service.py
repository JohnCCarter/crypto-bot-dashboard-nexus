...
class WebSocketCacheIntegrationService:
    ...
    def __init__(self, api_key: str, api_secret: str):
        ...
        self.data_freshness = {"balances": 0.0, "orders": 0.0, "positions": 0.0}
    ...
    async def _on_balance_update(self, balance: LiveBalance):
        ...
        try:
            ...
            current_balances = (
                self.cache.get_smart(balances_cache_key, "balances") or {}
            )
            ...
            logger.info(
                f"💰 Balance update via WebSocket: {balance.currency} = {balance.available} (NONCE SAVED)"
            )
    ...
    async def _on_order_update(self, order: LiveOrder):
        ...
        try:
            ...
            logger.info(
                f"📋 Order update via WebSocket: {order.symbol} {order.side} (NONCE SAVED)"
            )
    ...
    async def _on_fill_update(self, fill: OrderFill):
        ...
        try:
            ...
            logger.info(
                f"✅ Order fill via WebSocket: {fill.symbol} {fill.amount} @ {fill.price} (NONCE SAVED)"
            )
    ...
    def get_cached_balances(
        ...
        if balances:
            logger.info(
                f"📊 Serving cached balances (age: {age:.1f}s, WebSocket-populated)"
            )

        return balances

    def get_cached_orders(
        self, max_age_seconds: int = 15
        ...
        if orders:
            logger.info(
                f"📋 Serving cached orders (age: {age:.1f}s, WebSocket-populated)"
            )
    ...
    def should_use_rest_fallback(self, data_type: str, max_age: int = 60) -> bool:
        ...
        if not self.ws_connected or not self.ws_authenticated:
            logger.warning(
                f"🔄 WebSocket not connected, REST fallback required för {data_type}"
            )
            return True
        ...
        if age > max_age:
            logger.warning(
                f"⏰ WebSocket data för {data_type} too old ({age:.1f}s), REST fallback required"
            )
            return True
    ...
    def get_integration_stats(self) -> Dict[str, Any]:
        ...
        cache_efficiency = (
            self.stats.cache_updates_from_ws / max(1, total_updates)
        ) * 100
        ...
        for data_type, last_update in self.data_freshness.items():
            if last_update > 0:
                ...
                freshness_info[data_type] = {
                    ...
                    "is_fresh": age < 60,
                    "status": (
                        "fresh" if age < 30 else "stale" if age < 90 else "expired"
                    ),
                }
            else:
                freshness_info[data_type] = {
        ...
        return {
            "connection_status": {
                ...
                "last_heartbeat_seconds_ago": (
                    round(current_time - self.last_heartbeat, 1)
            ...
            "nonce_savings": {
                ...
                "estimated_savings_percent": min(
                    95,
                    (
                        self.stats.nonce_calls_saved
                        / max(
                            1,
                            self.stats.nonce_calls_saved
                            + self.stats.rest_fallback_calls,
                        )
                    )
                    * 100,
                ),
...
def get_websocket_cache_service(
    api_key: str = None, api_secret: str = None
) -> Optional[WebSocketCacheIntegrationService]:
    """
