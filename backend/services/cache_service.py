...
class EnhancedCacheService:
    ...
    CACHE_STRATEGIES = {
        ...
        "positions": {
            "ttl": 60,
            "type": "critical",
        },  # 60s för positions (ökat från 20s)
        "account_info": {"ttl": 600, "type": "critical"},  # 10 minuter för account info
        "trading_fees": {
            "ttl": 3600,
            "type": "critical",
        },  # 1 timme för fees (mycket statisk)
        "order_history": {
            "ttl": 180,
            "type": "standard",
        },  # 3 minuter för order history
        "open_orders": {
            "ttl": 15,
            "type": "volatile",
        },  # 15s för open orders (kan ändras)
        ...
        with self._lock:
            ...
            if key in self._cache:
                ...
                if current_time - entry.timestamp < effective_ttl:
                    ...
                    print(
                        f"📊 Cache HIT: {key} (age: {cache_age:.1f}s, "
    ...
    def set(
        ...
        with self._lock:
            ...
            if final_type == "critical" and effective_ttl >= 60:
                print(
                    f"🔑 CRITICAL cache saved: {key} - will reduce nonce consumption för {effective_ttl}s"
                )
    ...
    def _determine_cache_type(self, key: str) -> str:
        ...
        if any(
            ...
            for pattern in ["balance", "position", "account", "symbol", "fee"]
        ):
        ...
        with self._lock:
            ...
            return {
                ...
                "most_accessed": sorted(
                    access_stats, key=lambda x: x["access_count"], reverse=True
                )[:10],
                "cleanup_info": {
    ...
    def get_nonce_savings_estimate(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "estimated_api_load_reduction_percent": min(
                    90, (critical_hits / max(1, total_critical_requests)) * 100
                ),
                "cache_efficiency": {
                    "balances_cache_active": any(
                        "balance" in key for key in self._cache.keys()
                    ),
                    "positions_cache_active": any(
                        "position" in key for key in self._cache.keys()
                    ),
                    "account_info_cache_active": any(
                        "account" in key for key in self._cache.keys()
                    ),
                },
            }
