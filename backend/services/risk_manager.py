...
class RiskManager:
    ...
    def calculate_intelligent_position_size(
        ...
        if probability_data:
            ...
            probability_factor = (
                self.params.probability_weight * max(
                    probability_data.probability_buy,
                    probability_data.probability_sell
                ) +
            ...
            risk_score = probability_data.get_risk_score()
            risk_adjustment = 1.0 - (risk_score * 0.5)  # Max 50% reduction for high risk
            
            adjusted_position_size = base_position_size * probability_factor * risk_adjustment
        else:
        ...
        metadata = {
            ...
            "risk_score": probability_data.get_risk_score() if probability_data else None
        }
    ...
    def calculate_dynamic_stop_loss(
        ...
        if probability_data:
            ...
            confidence_adjustment = 2.0 - confidence_factor  # Lower confidence = tighter
            
            adjusted_stop_pct = self.params.stop_loss_pct * risk_adjustment * confidence_adjustment
    ...
    def calculate_dynamic_take_profit(
        ...
        if probability_data:
            ...
            take_profit_multiplier = 1.0 + (confidence_factor * 0.5)  # Up to 50% more aggressive
            
            adjusted_take_profit_pct = self.params.take_profit_pct * take_profit_multiplier
    ...
    def calculate_position_size(
        ...
        # Calculate available portfolio fraction
        used_capital = sum(
            pos.get("size", 0) * pos.get("entry_price", 0) for pos in current_positions.values()
        )
        available_capital = portfolio_value - used_capital
