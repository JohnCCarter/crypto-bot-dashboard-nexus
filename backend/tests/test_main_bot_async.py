...
def mock_live_data_service():
    ...
    service.validate_market_conditions.return_value = (True, "Market conditions are suitable for trading")
...
def mock_risk_manager():
    ...
    risk_manager.calculate_intelligent_position_size.return_value = (0.1, {
...
async def test_main_async_no_trading_window(
    ...
    with patch("backend.services.main_bot_async.ConfigService", return_value=mock_config_service), \
         patch("backend.services.main_bot_async.get_live_data_service_async", return_value=mock_live_data_service), \
         patch("backend.services.main_bot_async.get_risk_manager_async", return_value=mock_risk_manager), \
         patch("backend.services.main_bot_async.get_trading_window_async", return_value=mock_trading_window), \
         patch("backend.services.main_bot_async.Notifier", return_value=mock_notifier):
...
async def test_main_async_invalid_market_conditions(
    ...
    mock_live_data_service.validate_market_conditions.return_value = (False, "Market conditions are not suitable")
...
async def test_run_main_async():
    ...
    with patch.object(main_bot_module, 'main_async', new_callable=AsyncMock) as mock_main_async:
