...
async def test_fetch_live_ohlcv(mock_live_data_service):
    ...
    mock_live_data_service.exchange.fetch_ohlcv.assert_called_once_with(
        "BTC/USD", "5m", limit=100
    )
...
async def test_fetch_live_orderbook(mock_live_data_service):
    ...
    # Verify the exchange method was called
    mock_live_data_service.exchange.fetch_order_book.assert_called_once_with(
        "BTC/USD", 20
    )
...
async def test_validate_market_conditions(mock_live_data_service):
    ...
    # Call the method
    valid, reason = await mock_live_data_service.validate_market_conditions(
        market_context
    )
