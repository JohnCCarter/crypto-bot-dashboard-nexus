...
class TestAsyncServicesIntegration:
    ...
    def test_async_service_error_handling(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/orders", json=invalid_order_data, timeout=10
        )
        ...
        assert response.status_code in [
            ...
            422,
            500,
        ]  # 422 is valid for validation errors
    ...
    def _place_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/orders", json=order_data, timeout=10
        )
        if response.status_code == 201:
            order_result = response.json()
            return {"status_code": response.status_code, "order_id": order_result["id"]}
...
class TestAsyncServiceErrorRecovery:
    ...
    def test_service_recovery_after_errors(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/orders", json=invalid_data, timeout=5
        )
    ...
    def test_concurrent_error_handling(self):
        ...
        invalid_data_list = [{"invalid": f"data_{i}"} for i in range(5)]
