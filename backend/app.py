...
def init_services() -> Dict[str, Any]:
    ...
    risk_params_dict = {
        ...
        "stop_loss_pct": risk_config.get("stop_loss_percent", 2.0)
        / 100.0,  # Convert % to decimal
        "take_profit_pct": risk_config.get("take_profit_percent", 4.0)
        / 100.0,  # Convert % to decimal
...
def on_bitfinex_close(ws, close_status_code, close_msg):
    ...
    logger.warning(
        f"Bitfinex WebSocket closed: {close_status_code}"
    )  # Changed to warning
    ws_connection_status["connected"] = False
        ...
        while True:
            try:
                ...
                if ws_connection_status["connected"]:
                    logger.info(
                        "💚 System Health: All services operational - WebSocket active, API responsive"
                    )
                else:
                    logger.warning(
                        "💛 System Health: WebSocket disconnected, API running on REST fallback"
                    )
            except Exception as e:
