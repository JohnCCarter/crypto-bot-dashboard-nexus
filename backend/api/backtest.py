...
def run_ema_crossover_strategy(data: pd.DataFrame, parameters: Dict[str, Any]) -> Dict[str, Any]:
...
def run_rsi_strategy(data: pd.DataFrame, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Run strategy
        signals = run_strategy(data, period=period, overbought=overbought, oversold=oversold)
...
async def get_backtest_results(backtest_id: str):
    ...
    raise HTTPException(
        status_code=501,
        detail="Backtest result storage not implemented yet"
    ) 