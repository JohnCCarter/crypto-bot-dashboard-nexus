...
async def get_nonce_monitoring(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get nonce monitoring: {str(e)}"
        )


@router.get("/cache", response_model=Dict[str, Any])
async def get_cache_monitoring(cache=Depends(get_cache_service_dependency)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cache monitoring: {str(e)}"
        )
...
async def get_hybrid_setup_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get hybrid setup status: {str(e)}"
        )
