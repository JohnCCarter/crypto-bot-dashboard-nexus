...
class BitfinexWebSocketVerifier:
    ...
    async def subscribe_to_symbol(self, symbol: str):
        ...
        try:
            # Subscribe to ticker
            ticker_msg = {"event": "subscribe", "channel": "ticker", "symbol": symbol}
        ...
        try:
            ...
            if len(data) >= 10:
                ticker_info = {
                    ...
                    "spread_pct": ((float(data[2]) - float(data[0])) / float(data[6]))
                    * 100,
                }
                ...
                if (
                    not previous_ticker
                    or abs(
                        ticker_info["last_price"] - previous_ticker.get("last_price", 0)
                    )
                    > 0.01
                ):
                    logger.info(
                        f"📊 {symbol}: ${ticker_info['last_price']:.2f} (Vol: {ticker_info['volume']:.2f}, Spread: ${ticker_info['spread']:.2f})"
                    )
            ...
            if isinstance(data, list):
                if len(data) > 0 and isinstance(data[0], list):
                    ...
                    if bids and asks:
                        ...
                        logger.info(
                            f"📚 {symbol} Orderbook: Bid ${best_bid:.2f} | Ask ${best_ask:.2f} | Spread ${spread:.2f}"
                        )
                    ...
                    raise NotImplementedError(
                        "Incremental orderbook update logic is not implemented. Consider adding simplified update logic here."
                    )
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if data.get("event") == "info":
                    ...
                    logger.info(
                        f"✅ Subscribed: {channel} for {symbol} (Channel ID: {channel_id})"
                    )
    ...
    async def print_statistics(self):
        ...
        for symbol, ticker in self.ticker_data.items():
            print(
                f"  {symbol}: ${ticker['last_price']:.2f} (Vol: {ticker['volume']:.2f})"
            )
            print(
                f"    Bid: ${ticker['bid']:.2f} | Ask: ${ticker['ask']:.2f} | Spread: {ticker['spread_pct']:.3f}%"
            )
        ...
        for symbol, book in self.orderbook_data.items():
            if book["bids"] and book["asks"]:
                ...
                print(
                    f"  {symbol}: ${best_bid:.2f} - ${best_ask:.2f} ({levels} levels)"
                )
    ...
    async def data_quality_check(self):
        ...
        for symbol in self.test_symbols:
            ...
            if len(book["bids"]) < 10 or len(book["asks"]) < 10:
                issues.append(
                    f"{symbol}: Thin orderbook (Bids: {len(book['bids'])}, Asks: {len(book['asks'])})"
                )

        # Check connection health
        if self.connection_stats["error_count"] > 0:
            issues.append(
                f"WebSocket errors detected: {self.connection_stats['error_count']}"
            )

        if self.connection_stats["last_heartbeat"]:
            heartbeat_age = (
                datetime.now() - self.connection_stats["last_heartbeat"]
            ).total_seconds()
    ...
    async def run_verification(self, duration: int = 60):
        ...
        logger.info(
            f"🚀 Starting Bitfinex WebSocket verification for {duration} seconds..."
        )
